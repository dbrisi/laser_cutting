...
def kerf(f,spacingParam,shift, curvature = 1.5):
    ...
    f.write(f'<text x = "{xStartNW + (horizontalDim*INCH_TO_PIX_CONV)/2}" y = "{yStartNW + (verticalDim*INCH_TO_PIX_CONV)/1.1}" dominant-baseline= "central" text-anchor= "middle" font-size = "30px" fill = "blue">' + str(curvature) + ' </text> \n')
...
def main():
    ...
    kerf(f,3, shift*0*96)
    kerf(f,2.75, shift*1*96)
    kerf(f,2.5, shift*2*96)
    kerf(f,2.25, shift*3*96)
    f.write('</svg>')
    f.close()
